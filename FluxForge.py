...
def logo(name="FluxForge"):
    a=pyfiglet.figlet_format(name,font=random.choice(FONTS));p=random.choice(PALETTES)
    print("\n".join([ (p[i%len(p)]+ln) for i,ln in enumerate(a.splitlines()) ]));print(Style.DIM+Fore.YELLOW+"A terminal art toolbox — FluxForge\n")
def colorize(s,p,phase=0,shift=False):
    o="";L=len(p)
    for i,ch in enumerate(s):
        if ch.strip(): idx=(i+phase)%L if not shift else (i*2+phase)%L; o+=p[idx]+ch
        else: o+=ch
    return o
def fancy_text(inp=None):
    t=(inp or input("Text (blank→FLUX): ").strip() or "FLUX")
    try: art=pyfiglet.figlet_format(t,font=random.choice(FONTS))
    except: art=pyfiglet.figlet_format(t)
    p=random.choice(PALETTES);mode=random.choice(["morph","glitch","shadow","tilt","braille","plain"])
    if mode=="morph":
    ...
    if mode=="shadow":
        off=random.randint(1,3);Lns=art.splitlines();o1=[colorize(" "*off+ln,p,2,True) for ln in Lns];o2=[colorize(ln,p,0) for ln in Lns]
        out="\n".join(a+"\n"+b for a,b in zip(o1,o2+[""]*(len(o1)-len(o2)))); print(out); return out
    if mode=="tilt":
        t=random.choice([-2,-1,0,1,2]); out="\n".join(" "*max(0,i*t)+colorize(ln,p,i) for i,ln in enumerate(art.splitlines())); print(out); return out
    if mode=="braille":
        out="\n".join("".join(random.choice([Fore.MAGENTA+'⠿',Fore.CYAN+'⠄',Fore.YELLOW+'⠶']) if ch.strip() else " " for ch in ln) for ln in art.splitlines()); print(out); return out
    out="\n".join(colorize(ln,p,random.randint(0,5)) for ln in art.splitlines()); print(out); return out
def heart(size=18):
    s=max(6,min(40,size));W=int(s*2.8);H=int(s*1.6);out=[]
    for j in range(H,-H,-1):
        row=""
        for i in range(-W,W):
            x=i/(W/1.8); y=j/(H/1.0); val=(x**2 + y**2 -1)**3 - x**2 * y**3; row+=Fore.RED+"❤" if val<=0 else " "
        out.append(row)
    out="\n".join(out); print(out); return out
def spiral(n=19):
    ...
    while l<=r and t<=b:
        [ grid[t].__setitem__(i,chs[step%5]) for i in range(l,r+1) ]
        [ grid[i].__setitem__(r,chs[step%5]) for i in range(t+1,b+1) ]
        if t!=b: [ grid[b].__setitem__(i,chs[step%5]) for i in range(r-1,l-1,-1) ]
        if l!=r: [ grid[i].__setitem__(l,chs[step%5]) for i in range(b-1,t,-1) ]
...
def fractal_tree(levels=6):
    levels=max(3,min(10,levels));w=levels*7+18;h=levels*4+10;canvas=[[" "]*w for _ in range(h)]
    def branch(x,y,ang,depth,lenb):
        if depth<=0: return
        nx=int(x+math.cos(ang)*lenb); ny=int(y-math.sin(ang)*lenb)
        ch='|' if abs(nx-x)<2 else ('/' if nx<x else '\\')
        draw_line(canvas,w,h,x,y,nx,ny,ch)
        branch(nx,ny,ang-math.pi/6,depth-1,int(lenb*0.7)); branch(nx,ny,ang+math.pi/6,depth-1,int(lenb*0.7))
    branch(w//2,h-1,math.pi/2,levels,levels*3); out="\n".join("".join((Fore.GREEN+c if c!=" " else " ") for c in row) for row in canvas)
...
def sine_wave(sym="*",amp=6,freq=0.12,rows=12,frames=120):
    ...
    for t in range(frames):
        ...
        for x in range(w):
            y=int((math.sin(x*freq + t*0.28)+1)*(amp)); y=max(0,min(H-1,y)); buf[y][x]=Fore.CYAN+sym
...
def dvd(label="DVD",frames=240):
    ...
    for _ in range(frames):
        ...
        [ print(" "*x+color+label if r==y else "") for r in range(rows) ]
        x+=vx; y+=vy
...
def particles(frames=40):
    ...
    for _ in range(160): ang=random.random()*math.pi*2; sp=random.uniform(0.3,2.5); parts.append([cx,cy,math.cos(ang)*sp,math.sin(ang)*sp,random.choice(["*", "+", "o", "•", "✦"])])
    for _ in range(frames):
...
def puls_rings(frames=40):
    ...
    for t in range(frames):
        ...
        for y in range(rows):
            row=""; [ row.__iadd__(Fore.CYAN+"*" if abs(int(math.hypot(x-cx,y-cy))-ring)<1 else " ") for x in range(cols) ]
            print(row)
        time.sleep(0.07)
    return ""
...
def mandala(frames=60,rad=14):
    ...
    for t in range(frames):
        ...
        for y in range(H):
            row=""; 
            for x in range(W):
                d=math.hypot(x-cx,y-cy); val=math.sin((d/R)+t*0.15)
...
def hypnosis(frames=120,spirals=3):
    ...
    for t in range(frames):
        ...
        for s in range(spirals):
            for a in [i*0.12 for i in range(0,360)]:
                r=1+(s+1)*math.sin(a*0.1 + t*0.12)*(1+0.5*math.sin(t*0.2))
                x=int(cx + math.cos(a+t*0.05)*r*(s+3)); y=int(cy + math.sin(a+t*0.05)*r*(s+2))
...
def fire(frames=80,W=None,H=12):
    ...
    for _ in range(frames):
        ...
        print("\033[2J\033[H",end=""); [ print("".join(Fore.YELLOW+c if c in ".*o%@" else c for c in row)) for row in canvas ]; time.sleep(0.05)
    return ""
...
def ascii_clock():
    ...
    while time.time()<end:
        print("\033[2J\033[H",end=""); tm=time.localtime(); s=time.strftime("%H:%M:%S",tm); date=time.strftime("%d %b %Y",tm); art=pyfiglet.figlet_format(s,font=random.choice(FONTS));p=random.choice(PALETTES); print(colorize(art,p)); print(" "*max(0,(TC()-len(date))//2)+date); time.sleep(1)
    return ""
def random_art(iterations=10):
    ...
    for _ in range(max(1,min(20,iterations))):
        clear(); f=random.choice(funcs)
        try: (f(random.randint(6,20)) if f in (heart,spiral) else f())
        except: 
            try: f()
            except: pass
        time.sleep(0.6)
...
def mandelbrot(W=None,H=None,itermax=40):
    W=W or min(120,TC()); H=H or min(40,TL()-4); out=[]
    for j in range(H):
        row=""
        for i in range(W):
            x0=(i-W/2)/(W/4); y0=(j-H/2)/(H/4); x=0.0; y=0.0; it=0
            while x*x+y*y<=4 and it<itermax:
                xt=x*x-y*y + x0; y=2*x*y + y0; x=xt; it+=1
            chars=" .:-=+*#%@"; row+=chars[int(it/itermax*(len(chars)-1))]
        out.append(row)
    print("\n".join(out)); return "\n".join(out)
...
def spectrum(frames=120,bands=80):
    ...
    for _ in range(frames):
        row=""; 
        for i in range(bands): h=random.randint(1,H); row+=Fore.MAGENTA+"█"*h+Fore.RESET+" "
        print("\033[2J\033[H",end=""); print(row); time.sleep(0.06)
    return ""
def typewriter_banner():
    s=input("Banner text: ").strip() or "FLUX"; art=pyfiglet.figlet_format(s,font=random.choice(FONTS)); lines=art.splitlines()
    for i in range(1,len(lines)+1): print("\033[2J\033[H",end=""); print("\n".join(lines[:i])); time.sleep(0.18)
    return art
def maze(w=41,h=21):
    W=max(21,min(79,w)); H=max(11,min(39,h)); maze=[['#']*W for _ in range(H)]
    def carve(x,y):
        dirs=[(2,0),(-2,0),(0,2),(0,-2)]; random.shuffle(dirs)
        for dx,dy in dirs:
            nx,ny=x+dx,y+dy
            if 1<=nx<W-1 and 1<=ny<H-1 and maze[ny][nx]=='#':
                maze[ny][nx]=' '; maze[y+dy//2][x+dx//2]=' '; carve(nx,ny)
    maze[1][1]=' '; carve(1,1); print("\n".join("".join(row) for row in maze)); return "\n".join("".join(row) for row in maze)
    ...
    for _ in range(gens):
        ...
        for y in range(R):
            for x in range(C):
                ...
                for dy in (-1,0,1):
                    for dx in (-1,0,1):
                        if dy==0 and dx==0: continue; s+=g[(y+dy)%R][(x+dx)%C]
...
def save_out(txt):
    ...
    fn=input("Filename (art.txt): ").strip() or f"flux_art_{int(time.time())}.txt"
    if not fn.lower().endswith(".txt"): fn+=".txt"
    try:
        with open(fn,"w",encoding="utf-8") as f: f.write(stripc(txt)); print(Fore.GREEN+f"Saved: {fn}")
    except Exception as e: print(Fore.RED+"Save failed:",e)
def menu():
    last=""; clear(); logo("FluxForge")
    while True:
        try:
            print(Style.BRIGHT+Fore.GREEN+"\nMAIN MENU"); print("1) Fancy Text"); print("2) Shapes"); print("3) Animations"); print("4) Patterns"); print("5) Tools"); print("6) Save last .txt"); print("7) Exit"); ch=input(Fore.CYAN+"Choice: ").strip()
        except (KeyboardInterrupt,EOFError):
            print("\nbye"); break
        if ch=="1": clear(); last=fancy_text(None)
        elif ch=="2":
            clear(); print("SHAPES"); print("1) Heart"); print("2) Spiral"); print("3) Fractal Tree"); print("4) Sine Wave"); print("5) Mandala"); print("6) Maze")
            s=input("Pick: ").strip()
            ...
            clear(); print("ANIMATIONS"); print("1) DVD"); print("2) Matrix"); print("3) Particles"); print("4) Rings"); print("5) Lightning"); print("6) Fire"); print("7) Rain"); print("8) Hypnosis"); print("9) Starfield"); print("10) CRT Glitch"); print("11) Spectrum"); print("12) Lissajous")
            ...
            clear(); print("PATTERNS"); print("1) Mandala"); print("2) Mandelbrot"); print("3) Particle Field"); print("4) Conway"); print("5) Snow")
            p=input("Pick: ").strip()
            ...
            clear(); print("TOOLS"); print("1) ASCII Clock"); print("2) Braille Pixelizer"); print("3) Random Art"); print("4) Typewriter Banner"); print("5) One-line Minify Save")
            u=input("Pick: ").strip()
            if u=="1": last=ascii_clock()
            elif u=="2":
                txt=input("Enter small text: ").strip() or "A"; art=pyfiglet.figlet_format(txt,font=random.choice(FONTS))
                out="\n".join("".join("⠿" if c.strip() else " " for c in ln) for ln in art.splitlines()); print(out); last=out
            elif u=="3": last=random_art(10)
            elif u=="4": last=typewriter_banner()
            elif u=="5":
                s=input("Paste text to minify: ")
                m=re.sub(r'\s+',' ',s).strip(); fn=input("Filename (min.txt): ").strip() or "minified.txt"
                with open(fn,"w",encoding="utf-8") as f: f.write(m); print(Fore.GREEN+f"Saved: {fn}"); last=m
            else: print(Fore.RED+"Invalid.")
        elif ch=="6": save_out(last)
        elif ch=="7": print(Fore.YELLOW+"FluxForge signing off."); break
        else: print(Fore.RED+"Invalid.")
